package com.project.professor.allocation.controller.mapper;

import com.project.professor.allocation.dto.ProfessorCompleteDTO;
import com.project.professor.allocation.dto.ProfessorCreationDTO;
import com.project.professor.allocation.dto.ProfessorSimpleDTO;
import com.project.professor.allocation.entity.Professor;
import org.modelmapper.ModelMapper;
import org.springframework.stereotype.Component;

import java.util.List;
import java.util.stream.Collectors;

@Component
public class ProfessorMapper {

    private final ModelMapper modelMapper;

    public ProfessorMapper() {
        this.modelMapper = new ModelMapper();
    }

    public List<ProfessorSimpleDTO> toProfessorSimpleDTO(List<Professor> professors) {
        return professors.stream().map(this::toProfessorSimpleDTO).collect(Collectors.toList());
    }

    public ProfessorSimpleDTO toProfessorSimpleDTO(Professor professor) {
        return modelMapper.map(professor, ProfessorSimpleDTO.class);
    }

    public ProfessorCompleteDTO toProfessorCompleteDTO(Professor professor) {
        return modelMapper.map(professor, ProfessorCompleteDTO.class);
    }

    public Professor toProfessor(ProfessorCreationDTO professorCreationDTO) {
        return modelMapper.map(professorCreationDTO, Professor.class);
    }
}
